...
class Trade(Base):
    ...
    exit_qty:     Mapped[Optional[int]] = mapped_column(Integer, nullable=True)   # partial exit qty
    ...
    def compute_pnl(self) -> None:
        ...
        Uses exit_qty for P&L if set, otherwise falls back to qty.
        """
        if self.sell_price and self.buy_price:
            qty_for_pnl = self.exit_qty if self.exit_qty else self.qty
            self.pnl = round(
                float((self.sell_price - self.buy_price) * qty_for_pnl), 2
            )
            self.pnl_pct = round(
                float((self.sell_price - self.buy_price) / self.buy_price * 100), 2
            )
