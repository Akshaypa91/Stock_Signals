...
class TradeUpdate(BaseModel):
    ...
    exit_date:   date
    exit_qty:    Optional[int] = None   # partial exit — defaults to full qty
...
@router.put("/{trade_id}", summary="Close a trade (full or partial exit)")
async def close_trade(
    ...
    # exit_qty defaults to full qty if not provided
    exit_qty = body.exit_qty if body.exit_qty and body.exit_qty > 0 else trade.qty
    exit_qty = min(exit_qty, trade.qty)  # cap at total qty
    ...
    await db.refresh(trade)
    logger.info(
        "Trade closed: #%d %s exit_qty=%d PnL=%.2f (%.2f%%)",
        trade.id, trade.symbol, exit_qty, trade.pnl or 0, trade.pnl_pct or 0,
    )
    return trade.to_dict()
...
def _compute_summary(trades: list[Trade]) -> dict:
    ...
    days     = [t.hold_days for t in closed if t.hold_days is not None]
    avg_hold = round(sum(days) / len(days), 1) if days else None
