  ...
  return (
    <div className="bg-slate-900/80 border-b border-slate-700/40 overflow-hidden py-1.5">
      <div
        ...
        style={{ animationDuration: `${Math.max(20, active.length * 4)}s` }}
      >
        {items.map((sig, i) => {
          const ltp = getLtp(sig.upstox_key)
          const chg = ltp && sig.entry
            ? (((ltp - sig.entry) / sig.entry) * 100).toFixed(2)
            : null

          return (
            <span key={`${sig.id}-${i}`} className="inline-flex items-center gap-2 text-xs">
              <span className="text-slate-300 font-semibold">{sig.symbol}</span>
              <span className={`font-bold tabular-nums ${
                !ltp ? 'text-slate-500'
                : ltp >= sig.entry ? 'text-emerald-400'
                : ltp <= sig.sl ? 'text-red-400'
                : 'text-amber-400'
              }`}>
