...
const STATUS_LABELS = {
  active: 'Active', hit_t1: 'Hit T1', hit_t2: 'Hit T2', stopped: 'Stopped', time_stop: 'Time Stop',
}
...
function fmt(n) {
  return Number(n).toLocaleString('en-IN', { minimumFractionDigits: 1 })
}
...
export default function StockCard({ signal }) {
  ...
  return (
    <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-3 sm:p-4 hover:border-slate-500/70 transition-all">

      {/* Header */}
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-white font-bold text-base sm:text-lg tracking-wide">{signal.symbol}</span>
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${signal.strategy === 'S1' ? 'bg-violet-500/20 text-violet-300 border-violet-500/40' : 'bg-cyan-500/20 text-cyan-300 border-cyan-500/40'}`}>
              {signal.strategy}
            </span>
          </div>
          <div className="text-slate-400 text-xs mt-0.5">{signal.signal_date}</div>
        </div>
        <span className={`text-xs font-semibold px-2 py-1 rounded-full border flex-shrink-0 ${statusCls}`}>
      ...
      {/* LTP */}
      <div className="mb-3">
        <div className={`text-xl sm:text-2xl font-bold tabular-nums ${ltpColor(ltp, signal.entry, signal.sl)}`}>
      ...
      {/* Level grid — 2x2 on mobile, 4 cols on sm+ */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
        {[
          ...
          <div key={label} className="bg-slate-900/50 rounded-lg p-2 text-center">
            ...
            <div className={`font-semibold text-xs sm:text-sm tabular-nums ${cls}`}>₹{fmt(value)}</div>
          </div>
        ))}
      </div>

      {/* Stats */}
      <div className="flex items-center justify-between text-xs border-t border-slate-700/50 pt-2.5 flex-wrap gap-1">
        <div className="flex gap-3">
          <div>
            <span className="text-slate-500">SL </span>
            <span className={`font-semibold ${slPctColor(signal.sl_pct)}`}>{signal.sl_pct}%</span>
          </div>
          <div>
            <span className="text-slate-500">R:R </span>
            <span className={`font-semibold ${rrColor(signal.rr1)}`}>1:{signal.rr1}</span>
            <span className="text-slate-600 mx-0.5">/</span>
            <span className={`font-semibold ${rrColor(signal.rr2)}`}>1:{signal.rr2}</span>
          </div>
        </div>
