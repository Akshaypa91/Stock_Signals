...
function exportCSV(signals) {
  ...
  const rows = signals.map(s => [
    ...
    s.entry, s.sl, s.t1, s.t2, s.sl_pct, s.rr1, s.rr2, s.qty, s.status,
  ])
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-white text-2xl font-bold">Signal History</h1>
      ...
      {/* Filters */}
      <div className="flex flex-wrap gap-3 mb-6 p-4 bg-slate-800/50 rounded-xl border border-slate-700/50">
        <select value={filters.strategy} onChange={e => setF('strategy', e.target.value)}
          className="bg-slate-700 text-slate-200 text-sm rounded-lg px-3 py-2 border border-slate-600 focus:outline-none">
      ...
      <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading && (
                <tr><td colSpan={11} className="text-center text-slate-500 py-16">Loading…</td></tr>
              )}
              {!loading && signals.length === 0 && (
                <tr><td colSpan={11} className="text-center text-slate-500 py-16">No signals found</td></tr>
              )}
              {signals.map(sig => (
                <tr
                  key={sig.id}
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${
                      sig.strategy === 'S1' ? 'bg-violet-500/20 text-violet-300' : 'bg-cyan-500/20 text-cyan-300'
                    }`}>{sig.strategy}</span>
                  </td>
                  ...
                  <td className={`px-4 py-3 tabular-nums font-medium ${
                    sig.sl_pct <= 8 ? 'text-emerald-400' : sig.sl_pct <= 12 ? 'text-amber-400' : 'text-red-400'
                  }`}>{sig.sl_pct}%</td>
                  <td className={`px-4 py-3 tabular-nums font-medium ${
                    sig.rr1 >= 2 ? 'text-emerald-400' : sig.rr1 >= 1 ? 'text-amber-400' : 'text-red-400'
                  }`}>1:{sig.rr1} / 1:{sig.rr2}</td>
                  <td className="px-4 py-3 text-slate-300">{sig.qty} + {sig.qty_half}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${STATUS_BADGE[sig.status] || STATUS_BADGE.active}`}>
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          <div className="bg-slate-800 border border-slate-600 rounded-2xl p-6 w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              {[
                ...
                ['SL %', `${selected.sl_pct}%`, 'text-slate-300'],
                ['ATR', `₹${selected.atr}`, 'text-slate-300'],
                ['R:R (T1)', `1:${selected.rr1}`, 'text-slate-300'],
                ['R:R (T2)', `1:${selected.rr2}`, 'text-slate-300'],
                ['Qty (full)', selected.qty, 'text-slate-300'],
                ['Qty (half)', selected.qty_half, 'text-slate-300'],
              ].map(([label, value, cls]) => (
            ...
            <div className="mt-3 bg-slate-900/50 rounded-xl p-3">
              <div className="text-slate-500 text-xs mb-1">Upstox Key</div>
              <div className="text-slate-400 text-xs font-mono break-all">{selected.upstox_key}</div>
            </div>
          </div>
        </div>
      )}
